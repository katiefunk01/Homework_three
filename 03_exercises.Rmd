...
garden_harvest %>%
  ...
  distinct(vegetable, has_is,  .keep_all = TRUE)
```
...
> The month with the most frequent bike trips was October which is where bike rentals peaked. After this month, there was a steady decrease until there were the least amount of bike trips in the month of January.
...
Trips %>%
  ...
  labs(title = "Histogram of time of day of bike rentals",
       x = "Hour",
       y = "")
```
...
Trips %>%
  ...
  geom_point(aes(color = total_depart)) # change the color of this
```
...
Trips %>%
  ...
  left_join(Stations,
            by = c("sstation" = "name")) %>%
  ggplot(aes(x = long, y = lat, size = casual_percent)) +
